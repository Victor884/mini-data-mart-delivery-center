...
- 11 tabelas, 20 relacionamentos, 58 medidas, 9 páginas e 81 visuais;
- 5 páginas visíveis e 4 páginas de tooltip em `320 × 240`;
- 11 componentes HTML/CSS, 5 sidebars, 5 navegadores e 5 botões para limpar slicers;
- todos os 81 visuais dentro do canvas; sobreposição permitida somente para o painel de fundo da sidebar;
- navegadores verticais sem páginas ocultas/tooltips e referências de tooltip válidas;
- cada página de tooltip contém um único painel HTML consolidado, evitando valores ocultos pelo modo responsivo dos cartões nativos;
- credenciais armazenadas: **nenhuma**.

O validador oficial retorna onze avisos `PBIR_VISUAL_TYPE_UNKNOWN`, um para cada instância de **HTML Content (lite)**. É um aviso de catálogo do CLI para o visual customizado; não há erro de schema, formatação ou estrutura.
